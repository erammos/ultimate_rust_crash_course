...
fn expensive_sum(v: Vec<i32>) -> i32 {
    ...
    v.iter().filter(|s| *s % 2 == 0).map(|s| s * s).sum()
}
...
fn main() {
    ...
    // the child threads.
    let (tx, rx) = channel::unbounded();
    let rx1: channel::Receiver<&str> = rx.clone();
    let handle_a =  thread::spawn ( move || {
        pause_ms(10);
        for v in rx1.iter()
        {
            println!("Test handle a {}",v);
        }
    });
    let handle_b =  thread::spawn ( move || {
        for v in rx.iter()
        {
            println!("Test handle b {}",v);
        }
    });
    let values = vec!{"xaxa","elias","kostas","tester"};
    for v in values
    {
        pause_ms(2);
        tx.send(v).unwrap()
